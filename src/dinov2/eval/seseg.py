...
class AllSegmentor(nn.Module):
    def __init__(self, Segmentor_dict):
        super().__init__()
        self.Segmentor_dict = nn.ModuleDict()
        self.Segmentor_dict.update(Segmentor_dict)

    def forward(self, inputs):
        return {k: v.forward(inputs) for k, v in self.Segmentor_dict.items()}

    def __len__(self):
        return len(self.Segmentor_dict)
...
def setup_seg_decoder(sample_output, n_last_blocks_list, learning_rates, batch_size, num_classes=1000):
    linear_seg_dict = nn.ModuleDict()
    optim_param_groups = []
    # dim=768
    for n in n_last_blocks_list:
        for avgpool in [False, True]:
            for _lr in learning_rates:
                ...
                out_dim = create_linear_input(sample_output, use_n_blocks=n, use_avgpool=avgpool).shape[1]
                dim = out_dim
                seg_decoder=MaskTransformer(n_cls=2, patch_size=16, d_encoder=dim,
                ...
                optim_param_groups.append({"params": seg_decoder.parameters(), "lr": lr})

    linear_segmentors = AllSegmentor(linear_seg_dict)
    if distributed.is_enabled():
        seg_decoder = nn.parallel.DistributedDataParallel(seg_decoder)

    return linear_segmentors, optim_param_groups
