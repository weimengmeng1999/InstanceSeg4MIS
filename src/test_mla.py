    ...
    for sample in tqdm(dataset_it):
        ...
        if args['save']:
            ...
            with open(txt_file, 'w') as f:
                ori_im = sample['image']*255
                ins_color = ['green', 'yellow', 'blue', 'cyan', 'brown']
                # loop over instances
                color_flag = 0
                for id, pred in enumerate(predictions):
                    ...
                    ori_im = draw_segmentation_masks(ori_im.cpu().to(torch.uint8), masks=pred['mask'].to(torch.bool).cpu(), alpha=.5, colors = ins_color[color_flag])
                    color_flag += 1
                ...
                ins_im_name = base + '_ins.png'
                ori_im = torchvision.transforms.ToPILImage()(ori_im)
                ori_im.save(os.path.join(args['save_dir'], ins_im_name))
